 ...
 * @author 52351
 */
public class mostrarCuentasProveedor extends javax.swing.JPanel {

    /**
     * Creates new form mostrarLaboratorios
     */
    public void setMostrarCuentasProveedor(Main workspace) {
        mainPrincipal = workspace;
    }

    public mostrarCuentasProveedor() {
        initComponents();
          leyenda.setVisible(false);
          ContRegistro = new DAOCuentaProveedorImp().contRegistro();
    ...
    private void initComponents() {
        ...
        jTableMC.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "ID", "PROVEEDOR", "NO. CUENTA", "BANCO", "ESTATUS"
            }
        ));
        ...
        buscarPre.setText("BUSCAR CUENTAS PORVEEDOR");
    ...
    private void nuevaPreMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_nuevaPreMouseClicked
        ...
        agregarCuentasProveedor nuevaCuentaP = new agregarCuentasProveedor();
        nuevaCuentaP.setSize(mainPrincipal.getworkSpace().getSize());
        nuevaCuentaP.setVisible(true);
        nuevaCuentaP.setMainPrincipal(mainPrincipal);
        mainPrincipal.getworkSpace().add(nuevaCuentaP, BorderLayout.CENTER);
    ...
    private void jTableMCMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTableMCMouseClicked
        ...
        if (row > -1 && row <= jTableMC.getSelectedRow()) {
            ...
            modificarCuentasProveedor modificarcProve = new modificarCuentasProveedor();
            this.mainPrincipal.getworkSpace().removeAll();
            modificarcProve.setSize(mainPrincipal.getworkSpace().getSize());
            modificarcProve.setVisible(true);
            modificarcProve.setMainPrincipal(mainPrincipal);
            //Datos de la tabla, selecionar un row
            int id = Integer.parseInt(String.valueOf(this.jTableMC.getValueAt(row, 0)));
            String proveedor = String.valueOf(this.jTableMC.getValueAt(row, 1));
            String noCuenta = String.valueOf(this.jTableMC.getValueAt(row, 2));
            String banco = String.valueOf(this.jTableMC.getValueAt(row, 3));
            char estatus = String.valueOf(this.jTableMC.getValueAt(row, 4)).charAt(0);
            //Ingresan los datos de la tabla a la interfaz Modificar Laboratorio
            modificarcProve.ObtenerCuentaModificar(new CuentasProveedor(id, proveedor, noCuenta,banco, estatus));
                    

            this.mainPrincipal.getworkSpace().add(modificarcProve, BorderLayout.CENTER);
    ...
    private void buscarPreMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_buscarPreMouseClicked
        if (buscartxtPre.getText() == "" || buscartxtPre.getText() == null) {
            ...
            LinkedList<CuentasProveedor> pre = new DAOCuentaProveedorImp().busquedaPorNombre(Integer.parseInt(buscartxtPre.getText().toString()));
            Object listaDatos[][] = new Object[pre.size()][5];
            for (int i = 0; i < pre.size(); i++) {
                listaDatos[i][0] = pre.get(i).getIdcuentaProveedor();
                listaDatos[i][1] = pre.get(i).getProveedor();
                listaDatos[i][2] = pre.get(i).getNoCuenta();
                listaDatos[i][3] = pre.get(i).getBanco();
                listaDatos[i][4] = pre.get(i).getEstatus();
            }

            DefaultTableModel modelTable = new DefaultTableModel(
                    listaDatos,
                    new Object[]{"ID", "PROVEEDOR", "NO. CUENTA", "BANCO","ESTATUS"}) {
    ...
    public void MostrarDatosCuentasProv() {
        LinkedList<CuentasProveedor> cProv = new DAOCuentaProveedorImp().show(cambioPagina);
        Object listaDatos[][] = new Object[cProv.size()][7];
        this.segmentacion.setText("Página "+cambioPagina +" de "+ (int)ContRegistro+ " Páginas en total");
        for (int i = 0; i < cProv.size(); i++) {
            listaDatos[i][0] = cProv.get(i).getIdcuentaProveedor();
            listaDatos[i][1] = cProv.get(i).getProveedor();
            listaDatos[i][2] = cProv.get(i).getNoCuenta();
            listaDatos[i][3] = cProv.get(i).getBanco();
            listaDatos[i][4] = cProv.get(i).getEstatus();
        }

        DefaultTableModel modelTable = new DefaultTableModel(
                listaDatos,
                new Object[]{"ID", "PROVEEDOR", "NO. CUENTA",
                    "BANCO", "ESTATUS"}) {
            @Override
            public boolean isCellEditable(int row, int column) {
                return false;
